...
const club = () => {
  ...
  useEffect(() => {
    ...
    const letters = document.getElementById("lettersMoveUp").innerText.split('')
  ...
  const convertToWorldsSpans = (s) => {
    ...
    var spans = words.map((v, i) => {
      return (
        <span key={i} style={{ "--delay": i }} >{v}</span>
      )
      ...
      <div className='Club' >
        <div className="theClubContainerGrid">
          ...
          <section className="clubItem item3">
            <h2 ref={el => headingsRef.current[3] = el} className='wordsFadeUp'>{convertToWorldsSpans('Vogue Luxury Events is renowned for transforming space into theatrical, themed environment, by using the most innovative technologies in the industry')}</h2>
          </section>
          ...
          <section className='clubItem item5' >
            ...
            <div className="contentDiv">
              <h2 ref={el => headingsRef.current[5] = el} className='wordsFadeUp'>{convertToWorldsSpans('No event is too large or too small to accomplish. We aspire to create magical moments for your special day')}</h2>
            </div>
          </section>
          ...
          <section className='clubItem item4 relative' >
            <h2 className='title abs-title text-white'>
              Entertain
            </h2>
            <div className="imgDiv">
